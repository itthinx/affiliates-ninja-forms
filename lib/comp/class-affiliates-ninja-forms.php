...
class Affiliates_Ninja_Forms {
	...
	public static function ninja_forms_after_submission( $form_data ){
		...
		$options = get_option( Affiliates_Ninja_Forms_Integration::PLUGIN_OPTIONS , array() );
		...
		if ( class_exists( 'Affiliates_Referral_WordPress' ) ) {
			...
			$affiliate_id = $r->evaluate( $post_id, $description, $data, $base_amount, $amount, $currency, $ninja_forms_referral_status, Affiliates_Ninja_Forms_Integration::NINJA_FORMS_POST_TYPE );
		} else {
			$affiliate_id = affiliates_suggest_referral( $post_id, $description, $data, $amount, $currency, $ninja_forms_referral_status, Affiliates_Ninja_Forms_Integration::NINJA_FORMS_POST_TYPE );
		}
	}
		...
		if ( $affiliate_id ) {
			...
			if ( $n > 0 ) {
				foreach( $referrer_params as $params ) {
					...
					if ( count( $order_items ) > 0 ) {
						...
						foreach( $order_items as $order_item_id => $order_item ) {
							...
							if ( $rate = $rc->seek_rate( array(
									...
									'object_id'    => $post_id,
									'integration'  => Affiliates_Ninja_Forms_Integration::NINJA_FORMS_INTEGRATION_NAME
							) ) ) {
								$rate_id = $rate->rate_id;
								$type = Affiliates_Ninja_Forms_Integration::NINJA_FORMS_POST_TYPE;
					...
					$params['type']             = Affiliates_Ninja_Forms_Integration::NINJA_FORMS_POST_TYPE;
